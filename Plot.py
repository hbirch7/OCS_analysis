...
H_PD_Value.SetXTitle('Number of photons per pulse')
H_PD_Value.SetYTitle('Photodiode output')
c3.SetGrid()
c3.Update()
c3fname = "FPGA{}_CH{}_PD_ValuevsNPH.png".format(fpgaNum,PBNum)
c3.Print(c3fname)

c4 = TCanvas('c4','PD_Value',1920,1080)
H_PD_Value = f.Get("pdValue vs nph")  
H_PD_Value.Draw()
H_PD_Value.SetMarkerStyle(20)
Title = "Photodiode output versus Number of photons per pulse for FPGA{}CH{}".format(fpgaNum,PBNum)
...
c4.SetGrid()
c4.Update()
c4fname = "FPGA{}_CH{}_PD_ValuevsNPH.png".format(fpgaNum,PBNum)
c4.Print(c4fname)
